...
def hyperparameter_tuning(hyperparams, train_loader, test_loader, device):
    ...
    for params in product(*hyperparams.values()):
        ...
        model = CombinedModel(model1, model2, num_classes, num_securities)
        optimizer = torch.optim.Adam(model.parameters(), lr=param_dict['learning_rate'])
        criterion = nn.CrossEntropyLoss()
        writer = SummaryWriter()
        train_model(model, train_loader, criterion, optimizer, param_dict['num_epochs'], device, writer)
        kappa = evaluate_model(model, test_loader, device)
        writer.close()
        if kappa > best_kappa:
            best_kappa = kappa
